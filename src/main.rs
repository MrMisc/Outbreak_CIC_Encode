...
impl Zone_3D {
    ...
    fn eviscerate(
        ...
        println!(
            "We have {} hosts before to eviscerate, {} of them are contaminated",
            vector
                .iter()
                .filter(|&x| { !x.eviscerated })
                .collect::<Vec<_>>()
                .len(),
            vector
                .iter()
...
fn main() {
    ...
    println!(
        "We have a total of {} eviscerators spawned",
        eviscerators.clone().len()
    );
    ...
    let no_of_contaminated_hosts: u64 = hosts
        .clone()
        .into_par_iter()
        .filter(|x| x.motile == 0 && x.contaminated)
        .collect::<Vec<_>>()
        .len() as u64;
    let no_of_hosts: u64 = hosts
        .clone()
        .into_par_iter()
        .filter(|x| x.motile == 0)
        .collect::<Vec<_>>()
        .len() as u64;
    println!(
        "This many {} hosts are there, and {}% are contaminated as a start",
        no_of_hosts,
        (no_of_contaminated_hosts as f64) / (no_of_hosts as f64)
    );
